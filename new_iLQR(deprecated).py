...
class iLQR_solver(object):
    ...
    def update_matrix(self, step_i):
        ...
        delta_td_u = (-s/bw)*50*approx_e_f1/(approx_e_f1+1)**2*(u/bw+rtt) + \
                     ...
                     (-s/bw)*50*approx_e_f1/(approx_e_f1+1)**2*b/s
        delta_td_s = -(u/bw+rtt)*50*approx_e_f1/(approx_e_f1+1)**2*(u/bw+rtt) - \
                     -(u/bw+rtt)*50*approx_e_f1/(approx_e_f1+1)**2*b/s + \
                    -b/s**2/(approx_e_f1+1) 
        ...
        delta_tf_b_s = 2500*(-(u/bw+rtt))*approx_e_f1*(1-approx_e_f1)/(approx_e_f1+1)**3*f_1/s - \
                        f_1/s**2*50*approx_e_f1/(approx_e_f1+1)**2 + \
                        1/s**2/(approx_e_f1+1) + \
                        50*(-(u/bw+rtt))*approx_e_f1/s/(approx_e_f1+1)**2
        delta_tf_u_u = delta_tf_b_b * (-s/bw)**2
        delta_tf_u_s = -1/bw*(50*approx_e_f1/(approx_e_f1+1)**2*f_1/s) + \
                      (-s/bw)*(2500*(-(u/bw+rtt))*approx_e_f1/(approx_e_f1+1)**2*f_1/s + \
                      50*approx_e_f1/(approx_e_f1+1)*(-(u/bw+rtt)*s-f_1)/s**2 + \
                      (1+approx_e_f1+s*50*(-(u/bw+rtt)))/(s*(approx_e_f1+1))**2)
        ...
        delta_td_b_u = 2500*(-s/bw)*approx_e_f1*(1-approx_e_f1)/(approx_e_f1+1)**3*(u/bw+rtt) + \
                        ...
                        1/s*-50*(-s/bw)*approx_e_f1/(approx_e_f1+1)**2
        ...
        delta_c_b_b = self.w3*delta_tf_b_b + \
                      self.w4*((delta_l_b_b*approx_new_latency - delta_l_b*delta_l_b*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_b*delta_l_b*approx_new_latency**2)/(approx_new_latency+1)**3
        ...
        delta_c_b_u = self.w3*delta_tf_b_u + \
                      self.w4*((delta_l_b_u*approx_new_latency - delta_l_b*delta_l_u*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_b*delta_l_u*approx_new_latency**2)/(approx_new_latency+1)**3
        delta_c_b_s = self.w3*delta_tf_b_s + \
                      self.w4*((delta_l_b_s*approx_new_latency - delta_l_b*delta_l_s*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_b*delta_l_s*approx_new_latency**2)/(approx_new_latency+1)**3
        ...
        delta_c_ps_l = 0
        delta_c_ps_pu = 0
        delta_c_ps_ps = self.w2*2
        delta_c_ps_u = 0
        delta_c_ps_s = self.w2*-2
        ...
        delta_c_u_u = self.w1/u**2 + self.w2*2*(1-np.log(u/u_p))/u**2 + \
                      ...
                      self.w4*((delta_l_u_u*approx_new_latency - delta_l_u*delta_l_u*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_u*delta_l_u*approx_new_latency**2)/(approx_new_latency+1)**3 +\
                      2500.0*self.barrier_1*approx_e_f5 + \
                      2500.0*self.barrier_2*approx_e_f6 
        delta_c_u_s = self.w4*((delta_l_u_s*approx_new_latency - delta_l_u*delta_l_s*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_u*delta_l_s*approx_new_latency**2)/(approx_new_latency+1)**3
        ...
        delta_c_s_s = self.w3*delta_tf_s_s + \
                      self.w4*((delta_l_s_s*approx_new_latency - delta_l_s*delta_l_s*approx_new_latency)*(1+approx_new_latency) + 2*delta_l_s*delta_l_s*approx_new_latency**2)/(approx_new_latency+1)**3 + \
                      self.w5*2 + self.w6*2 + \
    ...
    def iterate_LQR(self):
        ...
        for ite_i in range(self.n_iteration):
            ...
            for step_i in range(self.n_step):
                ...
                new_next_b, new_next_l = self.sim_fetch(new_x[0][0], new_x[1][0], new_u[0][0], new_u[1][0], rtt, bw)               # Simulate to get new next state
                if LQR_DEBUG:
                    print("new x: ", new_x)
                    print("new b: ", new_next_b)
                    print("bew l: ", new_next_l)
                if step_i < self.n_step - 1:
                    new_xt_list[step_i+1] = [[new_next_b], [new_next_l], [new_u[0][0]], [new_u[1][0]]]
